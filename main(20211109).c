...
void BlackBox_TX(uint8_t number)
{
	...
	{
		int i,z;
		char Tx_DATA[] = "$BAB115Q,TEST,OK,00\n";
		uint8_t DS = sizeof(Tx_DATA)-1;
		//-------------------DATA---------------------//
		//--------------------------------------------//
	...
	{
		int i,j,z;
		char Tx_DATA[] = "$BAB115Q,REV1,0.00,000000,00\n";
		uint8_t DS = sizeof(Tx_DATA)-1;
		//-------------------DATA---------------------//
		for(j=0;j<(sizeof(REV_Setting)-1);j++)
		{Tx_DATA[j+14]=REV_Setting[j];}
		//--------------------------------------------//
		for(i=0;i<(DS-3);i++){Sum += Tx_DATA[i];}
		cs1=(Sum>>4)&0x0f;
		Tx_DATA[DS-3]=(cs1<10)?(cs1+'0'):((cs1-10)+'a');
	...
	{
		int i,j,z;
		char Tx_DATA[] = "$BAB115Q,DATA,000,000,0,0,000,00\n";
		uint8_t DS = sizeof(Tx_DATA)-1;
		//-------------------DATA---------------------//
		for(j=0;j<(sizeof(BAB_DATA)-1);j++)
		{Tx_DATA[j+14]=BAB_DATA[j];}
		//--------------------------------------------//
		for(i=0;i<(DS-3);i++){Sum += Tx_DATA[i];}
		cs1=(Sum>>4)&0x0f;
		Tx_DATA[DS-3]=(cs1<10)?(cs1+'0'):((cs1-10)+'a');
	...
	{
		int i,z;
		char Tx_DATA[] = "$BAB115Q,BITR,OK,00\n";
		uint8_t DS = sizeof(Tx_DATA)-1;
		//-------------------DATA---------------------//
		//--------------------------------------------//
		for(i=0;i<(DS-3);i++){Sum += Tx_DATA[i];}
	...
	{
		int i,z;
		char Tx_DATA[] = "$BAB115Q,PRC1,OK,00\n";
		uint8_t DS = sizeof(Tx_DATA)-1;
		//-------------------DATA---------------------//
		//--------------------------------------------//
		for(i=0;i<(DS-3);i++){Sum += Tx_DATA[i];}
	...
	{
		int i,j,z;
		char Tx_DATA[] = "$BAB115Q,DATP,000,000,0,0,000,00\n";
		uint8_t DS = sizeof(Tx_DATA)-1;
		//-------------------DATA---------------------//
		for(j=0;j<(sizeof(BAB_DATA)-1);j++)
		{Tx_DATA[j+14]=BAB_DATA[j];}
	...
	{
		int i,z;
		char Tx_DATA[] = "$BAB115Q,0000,ERR,00\n";
		uint8_t DS = sizeof(Tx_DATA)-1;
		//-------------------DATA---------------------//
		Tx_DATA[9]=B_Rx_Command[7];
		Tx_DATA[10]=B_Rx_Command[8];
		Tx_DATA[11]=B_Rx_Command[9];
		Tx_DATA[12]=B_Rx_Command[10];
		//--------------------------------------------//
		for(i=0;i<(DS-3);i++){Sum += Tx_DATA[i];}
		cs1=(Sum>>4)&0x0f;
		Tx_DATA[DS-3]=(cs1<10)?(cs1+'0'):((cs1-10)+'a');
...
{	
	...
	if(STATE_CHG!=0&&CHG_State==1){Percent_P=1,Percent_N=0; RepCap=Percentgauge*97.2;}
	else if(BAT_Volt<=115){Percent_N=1,Percent_P=0; RepCap=Percentgauge*97;}
	else if(CHG_ST=='1')
	{
		Percent_P=0,Percent_N=0;
		if(BAT_Volt>=114)
		{
			if(Percent_1s==1)
				{
		...
		Percentgauge = RepCap/97;
		if(Percentgauge>99){Percentgauge=99;}
	}
	else if(CHG_ST=='3')
	{
		Percent_P=0,Percent_N=0;
		...
		Percentgauge = RepCap/97;
		if(Percentgauge<1){Percentgauge=1;}
		else if(Percentgauge>=100){Percentgauge=100;}
	}
#ifdef JAY_TEST
  Percentgauge=I2C_Data[5];
	...
	else if(DIS_Lock==1)
	{
	...
	else
		{
		#ifdef JAY_TEST                
			//-----------------------------------------------//
			Curr2=I2C_Data[12];                                    //0x10h FullCap Register 
			Curr1=I2C_Data[13];
			D_Current=((0x10000-((Curr1<<8)+Curr2))/3.6);	// <-- 4.06 --> 3.49 --> 3.78 --> 3.65
			//-----------------------------------------------//
		#else
			//-----------------------------------------------//
			Curr1=I2C_Data[13];
			...
			BAB_DAT3[1]='0'+(D_Current%1000)/100;
			BAB_DAT3[2]='0'+(D_Current%100)/10;
			if (D_Current >= 30) BAB_DAT3[3]='0';
			else BAB_DAT3[3]='0'+(D_Current%10);
		}
	//---------------------------------Charge-curr.
	if(BAB_DAT2[4]=='1')
	{	
	...
	if(BAB_DAT2[4]=='1')
	{
		if(FullCap>=RepCap){ChgCap=FullCap-RepCap;}
		else ChgCap=0;
		
		if(Percentgauge==100){CHG_TIME=0;}
		else
		{
			#if 1
				#ifdef MODEL_BAB_115
...
void I2C_Datawrite_Reset(void)                                    //Datasheet 98page
{
	if(GPIO_ReadInputDataBit(I2C_PORT, SDA_PIN)!=0)
	{
		I2C_START();
		I2C_TxData(SLAVE_ADDRESS);      //6C
		I2C_TxData(Config2);                   
		I2C_TxData(Reset_L);
		I2C_TxData(Reset_M);
		I2C_STOP();
...
void I2C_Dataread(void)
{
	...
	{	
		#ifdef CHECK_PCM_STATE
			if (pcmStateEntered)                                             //battetry 용량이 12V 미만이면 1 이상이면 0   1757line
			{
...
void I2C_Setting(){
     	if(GPIO_ReadInputDataBit(I2C_PORT, SDA_PIN)!=0)
	{
		...
		I2C_START();
		I2C_TxData(0x16);   //Slave_address                   
		I2C_TxData(0xA5);            //16A5  nFullCapNom         
		I2C_TxData(0x20); //D8               //27D8   10200      4E20   20000
		I2C_TxData(0x4E); //27
		I2C_STOP();
			}
		else I2C_STOP();
...
{
	LCD_TX(C_ID_BTINFO);	//USART 2A¸·I Aa·A
	Delay_ms(20);
	LCD_TX(C_ID_BATDAT);	//USART 2A¸·I Aa·A
	Delay_ms(20);
}
#endif
/*------------------------------------------------------------------------------------------*/
...
#ifdef JAY_TEST     
   // I2C_Setting();                          //FullCapRep Change value\
    Delay_ms(10);
#endif

#ifdef FUELGAUGEENABLE
	I2C_Datawrite_Reset();
